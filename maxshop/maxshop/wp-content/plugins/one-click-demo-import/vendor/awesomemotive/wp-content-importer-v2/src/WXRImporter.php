	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( ! empty( $meta ) ) {
			foreach ( $meta as $meta_item ) {
				$key = apply_filters( 'import_post_meta_key', $meta_item['key'], $post_id, $data );
				$value = false;

				if ( '_edit_last' == $key ) {
					if ( isset( $this->mapping['user'][intval($meta_item['value'])] ) )
						$value = $this->mapping['user'][intval($meta_item['value'])];
					else
						$key = false;
				}
